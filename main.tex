...
\mg{Ideas to expand upon
\begin{itemize}
    \item process is fast and we don't need dissipation
    \item process is slow and we need conformal coordinates to follow the flow.
    \item Write down plasma physics stuff
    \item Critique Krichnan non application to the decaying case.
    \item Write down time dynamics of the pick
    \itme Effects of temperate on the viscosity 
\end{itemize}
For High temperature before neutrino decoupling in thermal equilibrium -- Reynolds does not really change anything it is always large.
After neutrino decapling and thar free stinging we get the increase in viscosity, so from QCD to EW viscosity increases (both conformal and physicals).
Initial conditions: Before established KL41 spectrum the transition processes also produces GWs.
Qustions: is there KL41 spectra from the beginnig and what is trasition time to this 
This is a summert of all the apprachies to the formulation 
}
