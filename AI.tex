...
\usepackage{physics}            % \dv, \pdv, \bra, etc.
...
\begin{equation}
  E^{-11/6}\,dE = -\Ceps\,S^{-1/3}\,dt,
\end{equation}

and integrating from $(t_0,\,E_0)$ to $(t,\,E)$:

\begin{equation}
  \int_{E_0}^{E} E'^{-11/6}\,dE'
  = \left[-\frac{6}{5}\,E'^{-5/6}\right]_{E_0}^{E}
  = \frac{6}{5}\left(E^{-5/6} - E_0^{-5/6}\right).
\end{equation}

Setting the right-hand side equal to $-\Ceps S^{-1/3}(t-t_0)$ and
solving for $E$:
...
\begin{equation}
  ...
  = \frac{10}{7}\left(E^{-7/10} - E_0^{-7/10}\right)
  = -\Ceps\,I^{-1/5}\,(t-t_0).
\end{equation}
...
\begin{equation}
  ...
  \taueta(t) = \taueta \left(\frac{t}{t_0}\right)^{11/10}, }
