...
and universal. Every analytic treatment---the aeroacoustic Gaussian-closure framework of
\cite{2007Gogoberidze,2008Kahniashvili} and both the stationary and decaying kernels of this
paper---gives the \emph{causal} infrared slope
\begin{equation}
...
\paragraph{Origin, and what would resolve it.} The causal $k^3$ of Eq.~\eqref{eq:ir-causal}
presumes the GW spectrum is the analytic super-horizon tail of a source whose equal-time
stress correlator is analytic (white) at $k\to0$. The developed turbulent stress in the
simulations is not: a freely decaying, finite-duration source builds up large-scale stress
correlations---an effectively white \emph{stress} spectrum---which, through the radiation
relation $\Omega_{\rm GW}\sim\Omega_T/k^2$ found in \cite{2019RoperPol}, yields
$\Omega_{\rm GW}\propto k$ rather than $k^3$. The missing ingredient is therefore the
\emph{temporal} model, not the spatial one: the equal-time, stationary Gaussian closure
common to \cite{2007Gogoberidze,2008Kahniashvili} (and retained in our kernels) cannot
generate the broadband, finite-duration stress that produces the flat infrared. Restoring the
finite source duration---the self-similar slow-time integration left open in
Sec.~\ref{sec:decaying-peak}---is the natural route to the $k^1$ law and is the principal
unresolved item between the analytic and numerical descriptions.
...
\begin{table*}[ht]
  ...
  \begin{tabular}{l l l l l}
    ...
    this paper (kernels) & Gaussian / power-law & $1.47Mk_0$ / $2.4k_0$ & $k^3$ & $k^{-5}$ / $k^{-8/3}$ \\
  ...
  of magnitude across the three works; the infrared slope is the discrepancy that is both
  universal across the analytic estimates ($k^3$, causal) and contradicted by the simulation
  ($k^1$). The infrared slope is not quoted explicitly by \cite{2008Kahniashvili} but is the
  generic causal value of their aeroacoustic Gaussian-closure framework. Slopes are in the
  $\Omega_{\rm GW}=d\rho_{\rm GW}/d\ln k$ convention.}
