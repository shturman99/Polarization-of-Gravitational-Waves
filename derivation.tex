...
verified model by model throughout, is that the pipeline factorises: the infrared
slope is determined by the spatial model alone, the ultraviolet slope by the
temporal model alone, and the peak is the only feature that responds to both. That
...
literature into questions with definite answers.
...
gravitational-wave step at all; it is about the stress, and it reduces to a single
assumption, $\Omega_{T}\propto\Omega_{M}$, which is inconsistent with the stress
being a self-convolution of the fluid spectrum. The convolution floors at white
...
\paragraph{The single assumption at fault.} That one substitution is the whole
discrepancy, and it cannot hold for a quadratic source. Because $T_{ij}\sim B_iB_j$ is
...
innocent of the infrared discrepancy; the lone bad step is $\Omega_{\rm T}\propto
\Omega_{\rm M}$. Our own kernels do not take it---they carry the double convolution
...
\begin{table*}[ht]
  ...
  and Sec.~\ref{sec:principal-discrepancy} localizes it to two identifiable steps: on the
  analytic side the substitution $\Omega_{\rm T}\propto\Omega_{\rm M}$, which a quadratic
  source forbids, and on both sides the question of which branch of the finite-duration
  response Eq.~\eqref{eq:window-factor} the observed band of $k$ lies in.
...
every peak, the slopes are $p^{3.00}$ (Batchelor band alone), $p^{3.01}$ (inertial
...
$4.14\times10^{-4}$) is not the sum of the inertial-only peak ($p=0.74$,
$2.57\times10^{-4}$, i.e.\ the $\add{1.49}M$ sweeping scaling) and the Batchelor-only
peak ($p=0.47$, $5.16\times10^{-5}$). The practical consequence for
...
outward from $p=0.47$ to $1.26$ for the $k^4$ band and from $0.74$ to $2.13$ for
...
within $2.98$--$3.08$ of the causal value for every $M$ and for both bands, so
...
flattens it to $13\%$ ($k^4$ band) and $19\%$ ($k^{-5/3}$ band) across two
...
reaching only $p_{\rm peak}=1.26$ at $M=3$ where $1.1\,M$ would predict $3.3$:
...
\begin{figure}[ht]
  ...
  $M^3$ prefactor dotted; both bands follow it to $13\%$ and $19\%$ over two
...
\begin{enumerate}\itemsep2pt
  ...
  with \emph{identically zero} infrared power, still radiates $p^{3.01}$, and it
...
\begin{table*}[ht]
  ...
  $H[S_{\rm K41},T_{\rm dec}]$  & $2.97$ & $2.32$ & $\omega^{-2}$ & yes & Roper Pol et al.~\cite{RoperPol:2019wvy} (peak) \\
