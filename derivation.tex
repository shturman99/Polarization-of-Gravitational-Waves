...
$\rho_{\rm GW}$       & GW energy density (Isaacson), $\langle\dot h_{ij}\dot h_{ij}\rangle/32\pi G$ & \eqref{A:uetc}\\
...
$R_{ij}(\br,\tau)$    & Real-space two-point velocity correlator & \eqref{A:pairs}\\
...
$\Sigma(r)$           & Real-space stress correlator, $\langle\mathcal{S}_{ij}(\br_1)\mathcal{S}_{ij}(\br_2)\rangle$ & \eqref{eq:rs2-spectrum}\\
...
$L(\br,\tau)$, $I(\br,\omega)$ & Real-space GW correlator at lag $\tau$ and its temporal transform & \eqref{eq:rs2-L}\\
...
\end{equation}
\begin{equation}
  h_{ij}''+2\frac{a'}{a}h_{ij}'-\nabla^2h_{ij}=16\pi G\,a^2\,\Pi^{\rm TT}_{ij}
  \label{A:wave}
\end{equation}
\begin{equation}
  \Pi^{\rm TT}_{ij}(\kb,\tau)=\Lambda_{ij,lm}(\khat)\,T_{lm}(\kb,\tau),
  \quad T_{lm}=w\,v_lv_m,
  \quad \Lambda_{ij,lm}=P_{il}P_{jm}-\tfrac12P_{ij}P_{lm},
  \quad P_{ij}=\delta_{ij}-\khat_i\khat_j
  \label{A:source}
\end{equation}
\begin{equation}
  a\to1:\qquad h_{ij}''(\kb,\tau)+k^2h_{ij}(\kb,\tau)=16\pi G\,\Pi^{\rm TT}_{ij}(\kb,\tau)
  \label{A:osc}
\end{equation}
\begin{equation}
...
\begin{equation}
  ...
  \;\Longrightarrow\;
  \widetilde\Pi(k,\omega)=\!\int\!d(\Delta\tau)\,e^{i\omega\Delta\tau}\Pi(k;\Delta\tau)=H_{ijij}(\kb,\omega)
  \label{A:stationary}
\end{equation}
\begin{equation}
