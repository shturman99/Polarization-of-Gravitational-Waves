...
\paragraph{Complete single-variable collapse.}
Equation~\eqref{eq:peak-ansatz} can be cast as an exact one-variable scaling. Writing
the spectrum on the sweeping variable $\xi=p/M=k/(M k_0)$ and using $p=M\xi$,
\begin{equation}
  \Omega_{\rm GW}(p;M)\;=\;A\,M^3(M\xi)^3\,G(\xi)\;=\;M^{6}\,\big[A\,\xi^3 G(\xi)\big],
  \label{eq:M6-collapse}
\end{equation}
so that $\Omega_{\rm GW}/M^{6}$ is a function of $\xi$ \emph{alone}: the $p^3$
infrared rise carries an extra factor $M^3$ beyond the $M^3$ prefactor, and the
correct compensation on the $\xi$ axis is therefore $M^{6}$, not $M^{3}$. This is
the strongest form of the scaling, collapsing the infrared rise \emph{and} the peak
onto the single universal curve $A\,\xi^3 G(\xi)$.
Figure~\ref{fig:peak_xM} demonstrates it: plotted against $\xi=k/(M k_0)$, the
$M^{6}$-compensated spectra for $M=0.03$--$1$ coincide over their entire range,
peaking together at $\xi_\ast\simeq1.47$; only the transonic $M=3$ curve departs,
as expected from the saturation discussed below. Equivalently, the natural
wavenumber variable for the stationary spectrum is not $k/k_0$ but the
sweeping-scaled $k/(M k_0)$, the ratio that holds the peak fixed.
...
For the amplitude, $\Omega_{\rm GW}=p^2 h_c^2$ with $h_c^2(p)=p\,H(p,p)$, while the
...
\paragraph{Collapsing one axis versus two.}
Once the scenario parameters are removed, two distinct normalisations of the
\emph{Mach} dependence are possible, and they answer different questions.
\emph{(i) Amplitude-only ($y$-axis) collapse.} Dividing the strain by $M^{3/2}$
(equivalently $\Omega_{\rm GW}/M^3$) while keeping the physical frequency
$f\propto p$ on the $x$-axis. From $h_c^2=M^3 p\,G(p/M)$ this gives
$h_c\,M^{-3/2}=\sqrt{p\,G(p/M)}$, which is $M$-independent \emph{only} in the
infrared ($G\!\to\!1$): the causal rises stack, but the peaks remain at their true
frequencies $f_{\rm peak}=f_0\,(1.47\,M)$ and \emph{slide} with $M$. This is the
convention of the standard strain plots (e.g.\ the $h_c\,M^{-3/2}$ ordinate of
\cite{2007Gogoberidze}): it preserves the physical observable---where in the GW
band each model peaks---which is exactly what one needs to confront a detector.
\emph{(ii) Full (both-axis) collapse.} Additionally rescaling the frequency by
$M$, $x\to\xi=p/M=f/(M f_0)$. Because $h_c^2=\Omega_{\rm GW}/p^2$ and
$\Omega_{\rm GW}=M^6\,U(\xi)$ with $f^2\propto(Mf_0)^2\xi^2$, the strain
compensation must change from $M^{3/2}$ to $M^{2}$:
\begin{equation}
  \frac{h_c}{M^{2}}=\frac{\sqrt{U(\xi)}}{\xi}=\text{function of }\xi\text{ alone}
  \qquad\Longleftrightarrow\qquad \frac{\Omega_{\rm GW}}{M^{6}}=U(\xi),
  \label{eq:strain-collapse}
\end{equation}
collapsing infrared rise \emph{and} peak onto one curve
(Fig.~\ref{fig:peak_xM}). The extra power---$3/2\!\to\!2$ in strain,
$3\!\to\!6$ in $\Omega_{\rm GW}$---is precisely the $M^3$ that the $p^3$ rise
contributes once the abscissa is measured in units of $M f_0$ rather than $f_0$.
This collapse is the right diagnostic for the \emph{theory}: it exhibits the
self-similarity and proves the cutoff is a function of $p/M$ only. It necessarily
\emph{discards} the physical peak frequency---the very information the
amplitude-only plot keeps. The two views are thus complementary: $y$-only for
observation, both-axis for the scaling argument.
...
\begin{figure*}[ht]
  ...
  $A\,\xi^3 G(\xi)$ of Eq.~\eqref{eq:M6-collapse}---infrared rise and peak
  together---because the $p^3$ rise contributes an extra $M^3$ on the $\xi$ axis.
