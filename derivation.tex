...
with $\Omega_{\rm GW}$ from the master spectrum~\eqref{eq:omega-diag},
...
\begin{figure}[ht]
  ...
  Panel (a) is the full numerical $H_{pq}$ of Eq.~\eqref{eq:Hijij-AppA-dimless}
  (Sec.~\ref{sec:kraichnan-k41}); panel (b) is the aeroacoustic $p\to 0$ limit
  (Kahniashvili et al.\ 2008, \cite{2008Kahniashvili}).  Both give the
...
\begin{figure}[ht]
  ...
  decorrelation, closed form of Eq.~\eqref{eq:dimless-delta-Kraichnan},
  $\Omega_{\rm GW}(p)\propto
  ...
  convolution of Eq.~\eqref{eq:dimless-delta-decay}, IR slope $\mathbf{0.3}$
  (flagged: the temporal-tail extra suppression further reduces the IR power).}
...
\begin{figure}[ht]
  ...
  of Eq.~\eqref{eq:dimless-white-Kraichnan}, matches the universal IR slope
  $\mathbf{3}$.  Panel (b): decay temporal model of
  Eq.~\eqref{eq:dimless-white-decay}, IR slope $\mathbf{1.3}$ (flagged:
  temporal-convolution tails pull power down at small $p$).}
