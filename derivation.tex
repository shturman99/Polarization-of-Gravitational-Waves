...
footing, and that the pipeline separates cleanly: the infrared slope is fixed by
the spatial model alone, the ultraviolet by the temporal model alone, and only
the peak responds to both. Two consequences follow. First, the causal infrared
...
its bandwidth, the Mach number, the choice of temporal model, and the removal of
the temporal sector altogether. We demonstrate this with a band-split experiment
in which the Kolmogorov range alone---an input with identically zero infrared
power---still radiates a clean $k^{3}$. The long-standing $k^{3}$-versus-$k^{1}$
tension between analytic predictions and simulations therefore cannot be resolved
on the spatial axis, and we localise it instead to a single assumption,
$\Omega_{T}\propto\Omega_{M}$, which the self-convolution of the stress forbids.
...
a finite lifetime and not specific to any decorrelation model. We close by arguing
...
approximation \cite{Kraichnan:1959,Kraichnan:1965,Kraichnan:1977,Gogoberidze:2007an},
...
theorem---a correlation function must have a non-negative transform---and
...
merely different normalisations, and that decade is where LISA is most sensitive. (The rate
...
The white-noise stress floor established above is exactly the explanation
...
Eq.~\eqref{eq:k-break}. (The estimate of $k_{\rm break}$ relative to $k_1$ is ours, from the
published run parameters; it is not stated in \cite{RoperPol:2019wvy}.)
...
the quadratic source, and the discrepancy must be resolved on the temporal axis,
not the spatial one.
...
producing a broad \emph{subviscous} magnetic range \cite{Schekochihin:2020aqu}. Since
...
$k_d$, nor the simulations, which are all at ${\rm Pm}=1$.
...
spanning several decades in $k$ is a large amount of stress that the current
...
\begin{enumerate}\itemsep2pt
  \item The turbulence-to-GW pipeline factorises. The infrared slope of
  $\Omega_{\rm GW}$ is set by the spatial source model alone, the ultraviolet slope
  by the temporal model alone, and only the peak responds to both
  ...
  quadratic source, not of the fluid spectrum. It is unchanged by the infrared slope
  of the input, by the infrared bandwidth, by the Mach number, by the choice of
  temporal model, and by deleting the temporal sector entirely. The band-split
  experiment of Sec.~\ref{sec:band-split} makes the point at its sharpest: the
  Kolmogorov band alone, an input with no infrared power at all, radiates
  $p^{3.01}$.

  \item Consequently the $k^{3}$-versus-$k^{1}$ tension cannot be resolved on the
  spatial axis. It localises to one assumption, $\Omega_{T}\propto\Omega_{M}$, which
  the self-convolution of the stress forbids: below the peak the convolution is
  dominated by antiparallel pairs near the peak and floors at white noise, and this
  holds for Gaussian fields as well (Sec.~\ref{sec:principal-discrepancy}).

  \item The displacement of the GW peak to the source scale is controlled by a single
  number, the slope of the source correlator at zero lag. Any source with a hard
  finite lifetime supplies it; the specific decorrelation model does not matter
  (Sec.~\ref{sec:decaying-peak}). The question that would overturn the source-scale
  peak is whether the correlator of decaying turbulence is differentiable at zero
  lag, not whether it follows any particular power law.

  \item The dissipation input is the one still open, and it is not the one the field
  assumes: the early Universe is neither Navier--Stokes nor ${\rm Pm}=1$
  (Sec.~\ref{sec:outlook}).
\end{enumerate}
