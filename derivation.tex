...
$\Omega_{\rm GW}\propto k^1$. Two scales therefore govern the infrared---the source lifetime
$T_{\rm em}$ and its correlation time $\tau_c$---and \emph{both} flatten the spectrum: a
fixed-coherence source over $[0,T_{\rm em}]$ has its $k^3\!\to\!k^1$ transition at
$k\sim1/\min(\tau_c,T_{\rm em})$, so the $k^1$ band widens as either $T_{\rm em}$ or $\tau_c$
grows. In the limiting case $\tau_c\gtrsim T_{\rm em}$ (coherent over the whole lifetime) the
$k^1$ law covers the entire sub-peak range $T_{\rm em}^{-1}\lesssim k\lesssim k_{\rm peak}$;
a short-lived or rapidly decorrelating source recovers the causal $k^3$ over most of it. The
deep infrared $k\to0$ is $k^3$ in every case. (The ratio $\tau_c/T_{\rm em}$
alone is not the control: lengthening $T_{\rm em}$ and lengthening $\tau_c$ both flatten the
slope yet move the ratio in opposite directions.)

\paragraph{Numerical confirmation.} Evaluating the self-similar slow-time kernel directly
bears this out [Fig.~\ref{fig:ir_resolution}], and an independent brute-force double-time
integration reproduces the kernel to better than $0.3\%$ and the slopes below to within
$0.05$. In the aeroacoustic limit,
where the infrared slope lives, the slope measured over the sub-peak band falls monotonically
toward the simulated value as the source is made more coherent---slower decay at fixed
lifetime [Fig.~\ref{fig:ir_resolution}, right]: a source held coherent over its lifetime
gives slope $1.1$---the DNS law (the analytic limit is exactly $1$)---while rapid
decorrelation steepens it back toward the causal $3$ ($2.5$ over the band when the source
decays within an eddy turnover, and $3.0$ in the deep infrared $k\to0$, which is causal
regardless). Self-similar decay interpolates, giving $k^{1.9}$ for slow decay and $k^{2.5}$
for fast decay. The same flattening appears in
the full-spatial kernel, and---crucially---the peak stays at the source scale
(Sec.~\ref{sec:decaying-peak}; $k\simeq2.2$--$2.4\,k_0$ across the coherence range): a
coherent, finite-duration source carries a $k^1$ infrared and a source-scale peak
\emph{simultaneously}, the two features Roper Pol et al.\ report together. The discrepancy is thereby resolved, not merely
...
rapidly decorrelating (or instantaneous-source) limit. Both are correct in their own regime;
which one a real electroweak-scale source realises is set by how coherent it remains over its
active lifetime---both its decay rate and its duration.
...
\begin{figure}[ht]
  ...
  limit is exact) as the decay rate is varied at fixed lifetime $T_{\rm em}$, plotted against
  the resulting correlation-time ratio $\tau_c/T_{\rm em}$; it interpolates monotonically
  between the causal $k^3$ and the simulated $k^1$. (Along this family slower decay flattens
  the slope; a longer lifetime at fixed decay flattens it too, so $\tau_c/T_{\rm em}$ is not
  itself the sole control.) Generated by \texttt{Notebooks/fullspatial\_selfsimilar.py}.}
...
\begin{table*}[ht]
  ...
  \begin{tabular}{l l l l l}
    ...
    this paper (kernels) & Gaussian / power-law / finite-$T_{\rm em}$ & $1.47Mk_0$ / $2.4k_0$ & $k^3\!\to\!k^1$ (source coherence) & $k^{-5}$ / $k^{-8/3}$ \\
