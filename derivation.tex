...
Figure~\ref{fig:roperpol_convention} shows this analytic GW spectrum at $M=1$,
together with the model fluid spectrum $\Omega_{\rm M}(k)/k=E(k)\propto k^{-5/3}$
(plotted, like the GW curve, only over the Kolmogorov inertial range $k\ge k_0$)
and the two slopes the literature reports for the GW inertial range, drawn as
reference guides: $k^{-11/3}$ from the simulation of \cite{2019RoperPol} (run
ini2) and $k^{-9/2}$ from \cite{2007Gogoberidze}. We plot no simulation data
points and defer a quantitative data-level comparison to future work. Both curves
are \emph{dimensionless}: the GW curve is the bare kernel with the source stress
...
the kernel omits (this is why the simulated $\Omega_{\rm GW}/k\sim10^{-13}$ lies
far below the fluid spectrum), and only the slopes and peak locations are to be
compared. In the infrared the analytic GW spectrum rises with the causal slope
$\Omega_{\rm GW}/k\propto k^{2}$ (i.e.\ $\Omega_{\rm GW}\propto k^3$, fitted slope
$k^{+1.9}$) and peaks near $k\simeq k_0$. Above the peak it steepens
\emph{continuously}, and at $M=1$ far more steeply than either reference: the
measured post-peak slope is $\simeq k^{-4.8}$---steeper than both Gogoberidze's
$k^{-9/2}$ and the simulation's $k^{-11/3}$---before the $k$-dependent sweeping
decorrelation bends it below any single power law. The analytic stationary kernel
therefore does \emph{not} reproduce the extended $k^{-11/3}$ range seen in the
simulation; at the simulations' Mach number it is markedly steeper, crossing that
slope rather than following it. This high-$k$ disagreement is precisely the
simulation--analytic tension that motivates this paper---and that the decaying
kernel of Sec.~\ref{sec:peak-location} substantially relieves.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\columnwidth]{roperpol_convention_spectra}
  \caption{Analytic spectra in the Roper Pol et al.~\cite{2019RoperPol}
  convention $\Omega(k)/k=E(k)$ (energy per logarithmic wavenumber divided by
  $k$). Orange: analytic GW spectrum
  $\Omega_{\rm GW}(k)/k=p^2H_{ijij}(p,p;M,R)$ [Eq.~\eqref{eq:omega-over-k},
  stationary kernel Eq.~\eqref{eq:Hijij-AppA-dimless}, $M=1$, $R=10^4$]. Black:
  model fluid spectrum $\Omega_{\rm M}/k=E(k)\propto k^{-5/3}$. Both are shown only
  over the Kolmogorov inertial range $k\ge k_0$ and are \emph{dimensionless}---the
  GW curve is the bare kernel, so the GW--fluid vertical offset is not physical
  (only slopes and peak location are comparable). Dashed: fitted causal infrared
  slope $k^{+1.9}$. Dash-dot/dotted: the $k^{-11/3}$ (Roper Pol, run ini2) and
  $k^{-9/2}$ (Gogoberidze) slopes \emph{reported} in the literature, as references
  only---no simulation data points are plotted. At $M=1$ the analytic GW spectrum
  steepens past the peak to $\simeq k^{-4.8}$, steeper than both references.
  Generated by \texttt{Notebooks/roperpol\_convention\_spectra.py}.}
