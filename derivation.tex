...
%%% ---- round-2 referee comments (2026-08-17) ------------------------------
%%% \rev{...}   a referee comment, rendered in blue and boxed
%%% \revX{...}  a BLOCKING referee comment, rendered in a red-ruled box
%%% Set \refcommentstrue to hide them all.
\usepackage[most]{tcolorbox}
\newif\ifrefcomments\refcommentstrue
\ifrefcomments
  \newcommand{\rev}[1]{\par\noindent\begin{tcolorbox}[colback=blue!4,colframe=blue!55,
    boxrule=0.5pt,left=3pt,right=3pt,top=2pt,bottom=2pt,breakable]
    \footnotesize\textcolor{blue!65!black}{\textbf{[Referee, round 2]} #1}\end{tcolorbox}\par}
  \newcommand{\revX}[1]{\par\noindent\begin{tcolorbox}[colback=red!5,colframe=red!70!black,
    boxrule=1pt,left=3pt,right=3pt,top=2pt,bottom=2pt,breakable]
    \footnotesize\textcolor{red!70!black}{\textbf{[Referee, round 2 --- BLOCKING]} #1}\end{tcolorbox}\par}
\else
  \newcommand{\rev}[1]{}\newcommand{\revX}[1]{}
\fi
%%% ------------------------------------------------------------------------
...
decorrelation. We close on the third input, the dissipation
microphysics. The early Universe is neither Navier--Stokes nor of unit magnetic Prandtl
number, and no existing calculation or simulation of the GW signal uses the correct one;
we compute what that omission costs. At ${\rm Pm}\gg1$ the magnetic field cascades past
the viscous scale to the resistive one, and the stress it carries there does radiate --
but only above twice the viscous wavenumber, leaving the infrared unchanged to a few per
cent even at ${\rm Pm}=10^{8}$. The accompanying viscous contribution to the correlator
is flat below the inverse decorrelation time and so adds no power of $k$ where the
detectors look. The missing physics is real, and it is unobservable.
\end{abstract}
...
\paragraph{\add{What the caveat does and does not establish.}}\del{The decorrelation shape, not the onset.}
\rev{This heading asserts the negation of what the forward-model paragraph of this
same section now concludes---that the peak is displaced by the source starting and
stopping, not by the shape of the decorrelation. Delete or rewrite; it carries no
\texttt{\textbackslash del}.}
...
$p_{\rm peak}\simeq2.4$ for every $M$ and passes through the simulation point\add{---in
\emph{shape}. That must be squared with Sec.~\ref{sec:decaying-peak}, which finds the same
cusped kernel overpredicting the radiated \emph{energy} of the same run by one to two
orders of magnitude. The two are compatible only because they measure different things:
Fig.~\ref{fig:gw_peak_vs_mach} compares peak positions with the total energy normalised
away, and that normalisation is precisely what the forward model tests. The cusped kernel
puts the peak in the right place for the wrong reason, so what survives here is the
negative half---the sweeping law is excluded by an order of magnitude---and not the
positive attribution to decorrelation}; the
...
\section{The magnetic stress correlator has no usable cusp}
\label{sec:magnetic-uetc}
\why{R7: promoted from a paragraph inside Sec.~\ref{sec:peak-location}; both round-2 referees identified this as the paper's strongest result and noted it was filed as a caveat inside a section arguing the opposite}

\add{The question can be
...
Three limits of the test should be stated. It is not closure-free: propagating
$\Pi(k,t)$ forward uses the same Gaussian (random-phase) closure as the rest of this
paper. It discriminates \emph{asymmetrically}---the coherent and Gaussian-sweeping
columns are not separated from one another, so the test excludes a cusp but does not
measure the correlator. And it is a single run, at ${\rm Pm}=1$ and $R=158$, with an
\emph{imposed} initial field, so the switch-on whose window sets the spectrum is a
feature of the setup rather than of a physical transition; carrying the conclusion to a
phase transition needs either an argument or a second run, and we have neither.}
...
wrong. \add{The discriminant is \emph{idempotency of the window}, not the level at
which the memory is imposed. For a hard lifetime the field correlator is $R=1$ inside the
window, so the stress carries $R^{2}=R$---squaring does nothing---and the two-leg factor
is the un-squared tent, break $2.33/W$. The squared tent and its $\pi$ describe a
stationary triangular \emph{lag memory}, a different source that should not be called a
lifetime. Every closure in this paper imposes a lifetime, so $2.33$ is the coefficient
throughout and Eq.~\eqref{eq:burst-equals-triangle} governs.}
\del{Where a lifetime is imposed on the stress we use $2.33$, and where it is a property of the field we use $\pi$.}
\revX{This rule is \emph{wrong}, and it is load-bearing. The discriminant is
idempotency of the window, not stress- versus field-level memory. For a hard lifetime the
field correlator is $R=1$ inside the window, so the stress carries $R^{2}=R$: squaring
does nothing, the two-leg factor is the \emph{un-squared} tent, and the break is
$2.33/W$. (Checked: $|\int_0^W e^{i\omega t}dt|^{2}=4\sin^{2}(\omega W/2)/\omega^{2}$ to
ratio $1.000000$.) The squared tent and its $\pi$ belong to a \emph{stationary triangular
lag memory}---a different source, which must not then be called a lifetime. Consequences:
Table~\ref{tab:k-break-hz} is high by $\pi/2.33=1.35$ throughout; the corner counting in
Sec.~\ref{sec:k-break-hz} reverses for one corner ($5.03\to3.7\,$mHz, \emph{inside} the
LISA window); the abstract's exception clause rests on a $9\%$ margin against a $35\%$
dispute; and the factor $\pi$ is the one novelty that section claims for itself.}
...
What it does not test is equally worth stating. The band lies above the window break \add{$2.33/T_{\rm em}=0.058$} but
...
the magnetic unequal-time correlator\add{, which
Sec.~\ref{sec:decaying-peak} now constrains indirectly through a parameter-free forward
model}.}
...
test available. \add{We carry it out in Sec.~\ref{sec:decaying-peak}: a direct
time-domain measurement is beyond the published cadence, but the simulation's own GW
field supplies the transform at $\omega=k$, and the resulting bound excludes a usable
cusp.}
\del{, and we propose it here as the natural next step.}
...
manuscript's own---sit on the asymptotic branch and give $+0.57$ to $+0.77$ \add{(the eddy rows alone give $+0.70$--$+0.77$; the global
$\hat\tau=16.6$ closure, which lies inside our own $\tau_c$ range, is the shallowest at
$+0.570$)}, which is
...
$\pm0.002$. \add{The whole deficit is our spatial factor rolling over towards the peak,
and that deserves a word, because taken at face value it says the spatial model is wrong
near the peak---which sits awkwardly beside our organising claim that the spatial axis is
the settled one. The claim is narrower than it sounds: what Sec.~\ref{sec:band-split}
establishes is that no spatial choice moves the \emph{asymptotic infrared}. It says
nothing about the peak, which Sec.~\ref{sec:model-space} attributes to both axes, and the
band in question here is $0.66$ decades wide and centred on the roll-over. The honest
statement is that the spatial axis is settled where we tested it and untested where the
disagreement lies.} \del{The whole deficit is our spatial factor rolling over towards the peak
...
\rev{Reproduces. But ``$+0.70$ to $+0.77$'' elsewhere in this section quotes the eddy
rows only and omits the global $\hat\tau=16.6$ closure at $+0.570$, which lies inside the
manuscript's own $\tau_c$ range; residuals are $-0.526$ to $-0.728$.}
...
Sec.~\ref{sec:cps-reproduction} reaches for the simulation box. Two corners,
$\gamma\sim10^{-2}$ with $u_0\sim0.3$ and $\gamma\sim10^{-3}$ with $u_0\sim0.1$, put the
break at $1.1$ and $3.7\,$mHz, inside the window. Only one,
$\gamma\sim10^{-3}$ with $u_0\sim0.3$, pushes it to $11.2\,$mHz, above the window, so
that LISA would observe the causal side instead. That single corner is the one case in
which the $k^{3}$ asymptote is the observable rather than an inaccessible limit.}
...
$k_{\rm br}=1/\delta t_{\rm fin}$; what is added here is the coefficient $2.33$ of
\revX{This is the sentence that makes the blocking comment fatal rather than cosmetic:
the paper's one claimed novelty in this section \emph{is} the disputed coefficient.}
...
replacing Batchelor $k^4$ by Saffman $k^2$ leaves the slope at \add{$p^{3.00}$} and
...
$\xi_\ast M$ with $\xi_\ast=\add{1.488}$ (Eq.~\eqref{eq:peak-condition}; the fit
Eq.~\eqref{eq:peak-fit} reads $1.515\,M^{1.007}$) recovered from that band alone, and at a
...
non-helical MHD, the Hosking integral
\cite{Hosking:2020wom,Hosking:2022umv}\add{---whose conservation and the decay law that
follows from it have since been confirmed in direct simulation
\cite{Zhou:2022xhk,Brandenburg:2023rrd} and established analytically for local and
Poisson-type gauges \cite{Hew:2025zyh}}---
...
$k_d$. \add{We have now performed that calculation, and the answer is a clean
negative for observability.

\paragraph{\add{The subviscous range radiates, but only in the far ultraviolet.}}
Take the magnetic spectrum to follow Kolmogorov on $k_0<k<k_\nu$ and the
Kulsrud--Anderson form $E_M\propto k^{3/2}K_0(k/k_\eta)$ on $k_\nu<k<k_\eta$, with
$k_\eta=k_\nu\sqrt{\rm Pm}$ \cite{Kulsrud:1992rk,Schekochihin:2002xf,Schekochihin:2003bg},
matched continuously at $k_\nu$, and feed it through the kernel of
Sec.~\ref{sec:kraichnan-k41} with the source ceiling moved from $k_d$ to $k_\eta$.
Holding the large-scale field fixed and adding the tail,
Fig.~\ref{fig:subviscous-gw}(a) shows the contribution appearing only above
$p\simeq2k_\nu/k_0$: at $p=0.01$ and $p=0.1$ the truncated and ${\rm Pm}=10^{8}$
spectra agree to two to three per cent, while by $p\sim2k_\eta/k_0$ the subviscous
spectrum exceeds the truncated one by many orders of magnitude. That is what a
convolution must do---two legs near $k_\nu$ can only make gravitational waves at
$k\lesssim2k_\nu$---but it settles the question the previous paragraph left open:
\emph{the discarded stress is discarded from a band no detector observes.}

\paragraph{\add{And the viscous cusp does not rival the sweeping cutoff.}}
The temporal half must be done with the correct leg wavenumbers. Both legs damp,
so $f_2=f_{\rm sw}(k_1)f_{\rm sw}(k_2)e^{-\nu(k_1^{2}+k_2^{2})|\tau|}$ and
$f_2'(0^{+})=-\nu(k_1^{2}+k_2^{2})$, giving
$T\to2\nu(k_1^{2}+k_2^{2})/\omega^{2}$---not the $4\nu$ of the estimate above, which
puts both legs at the \emph{gravitational-wave} wavenumber. In the infrared the legs
sit near the dissipation scale, so the ratio of the two is
$(k_1^{2}+k_2^{2})/2k^{2}$, nominally $10^{6}$ at $p=10^{-2}$. That number is
misleading, and the correction runs the other way: the $\omega^{-2}$ branch is the
\emph{large}-$\omega$ asymptote of the cusp, valid only for $\omega\tau_c\gg1$.
Computing $T$ rather than extrapolating it, Fig.~\ref{fig:subviscous-gw}(b),
$T$ is \emph{flat} below $1/\tau_c\simeq20$---$0.1034$ at $p=10^{-2}$, $10^{-1}$ and
$1$ alike---and reaches the asymptote only by $p\sim10^{3}$. A flat temporal factor
carries no power of $k$, so the causal $k^{3}$ stands and the floor never replaces
the sweeping cutoff in the observable band.

The two halves agree, and they agree on a negative: the ${\rm Pm}\gg1$ physics is
real, is absent from every existing calculation, and lives entirely in the far
ultraviolet. Whoever wants it for a detector will not find it there. Validated in
\texttt{Notebooks/subviscous\_gw.py}: the spectrum is continuous at $k_\nu$, the
${\rm Pm}\to1$ limit reproduces the truncated kernel exactly, the fixed-energy
normalisation holds to $10^{-8}$, the two-leg asymptote is recovered to better than
$0.4\%$, and the kernel is converged to $0.1\%$ against grid refinement.}
\why{R-viscous: Sec.~VII promised this calculation and did not perform it; both
round-2 referees noted that the one unpre-emptable claim was uncomputed}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\columnwidth]{subviscous_gw}
  \caption{\add{(a) Adding a ${\rm Pm}\gg1$ subviscous magnetic range to a fixed
  large-scale field changes $\Omega_{\rm GW}$ only above $p\simeq2k_\nu/k_0$; the
  infrared is untouched. (b) The two-leg viscous temporal factor, computed rather
  than extrapolated: flat below $1/\tau_c$, so it adds no power of $k$ where the
  observations are.}}
  \label{fig:subviscous-gw}
\end{figure}

\add{One caution}
applies. \add{In the Kulsrud--Anderson picture \cite{Kulsrud:1992rk} the subviscous
\rev{The caveat should also cover the leg-wavenumber ambiguity: the crossover figure
compares a $p$-independent floor against a sweeping factor evaluated for legs at $k$,
while the peak plotted on the same axes comes from legs at the outer scale. Outer-scale
legs give $\hat T\to4M/(Rp^{2})$, not a constant---so the $p$-scaling, not just the
crossover, is undetermined until the full kernel is done. Fig.~\ref{fig:viscous-cusp-floor}(a)
should be read with that in mind: the floor and the sweeping cutoff drawn there are
evaluated for legs at different wavenumbers and are not strictly commensurable.}
...
\rev{The second half of this item still carries the sentence deleted from
Sec.~\ref{sec:k-break-hz} (``every corner but one''). The grid gives 6 below / 1 inside /
2 above. Round one's ``abstract contradicted by Appendix~G'' recurring inside a single
conclusion.}
...
\rev{Every $f_{\rm break}$ here inherits the $\pi$ of the blocking comment above and
is high by $1.35$ if the source is a window rather than a triangular memory. Recompute
before quoting any of it.}
  ...
  \add{has now been made, indirectly: the parameter-free forward model of
  Sec.~\ref{sec:decaying-peak} excludes a usable cusp in the magnetic stress correlator
  for the one published run it can address, so the peak there is a window effect.}
  \del{has not been made, and this conclusion is conditional on it.} \add{The nearest
\rev{Stale: Sec.~\ref{sec:decaying-peak} now reports the parameter-free forward model,
which the abstract elevates to ``we decide it against the decorrelation route''. This
conclusion still says the measurement has not been made, and does not mention it.}
  ...
  \add{We close the gap here rather than merely identifying it, and the answer is
  negative for observability: the subviscous stress radiates only above
  $p\simeq2k_\nu/k_0$, leaving the infrared untouched to a few per cent even at
  ${\rm Pm}=10^{8}$, and the viscous temporal factor is flat below $1/\tau_c$ so it
  adds no power of $k$ where the detectors look
  (Sec.~\ref{sec:outlook}, Fig.~\ref{fig:subviscous-gw}). The ${\rm Pm}\gg1$ physics is
  real and is missing from every existing calculation; it is also, on this
  calculation, unobservable. That is a weaker claim than we expected to make, and it
  is the one the calculation supports.}
  \del{We identify the gap here but do not close it; the size of the discarded
  contribution is not yet computed, and until it is, this is a programme rather than
  a result.}
\end{enumerate}
...
%%% Appendix B retained in the concise build (51 cross-references depend on it)
...
\end{tabular}
\end{center}
%%% Appendices D/E retained in the concise build (19 cross-references depend on them)
