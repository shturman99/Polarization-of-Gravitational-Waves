...
%%% Set \reviewmarkupfalse to compile a clean copy with all markup resolved.
\usepackage{multirow}
...
\begin{abstract}
The gravitational-wave (GW) background sourced by turbulence in the early
Universe is computed from three inputs: the spatial spectrum of the fluid, the
unequal-time correlator (UETC) of the source, and the dissipation physics that
terminates the cascade. We show that these three inputs are not on an equal
footing, and that the pipeline separates in its \emph{asymptotes}: the strict
$k\to0$ infrared slope is fixed by the spatial model alone, the ultraviolet by the
temporal model alone, and every intermediate feature---the peak and the break
alike---responds to both. Two consequences follow. First, the causal infrared
$\Omega_{\rm GW}\propto k^{3}$ is a property of the \emph{quadratic} source and
not of the fluid spectrum: it survives changing the infrared slope of the input,
its bandwidth, the Mach number, the choice of stationary temporal model, and the
removal of the temporal sector altogether. That the stress self-convolution floors at white
noise is known \cite{Caprini:2006jb,Caprini:2009fx,Brandenburg:2019uzj}; we verify
it \add{through the gravitational-wave kernel rather than the estimate
$\Omega_{\rm GW}\propto F(k)/k^{2}$, and with the temporal sector retained, which
Ref.~\cite{Brandenburg:2019uzj} explicitly sets aside}, by a band-split experiment in
which the Kolmogorov range alone---an input with identically zero infrared
power---still radiates a clean $k^{3}$.
\del{by a sharper test,} The $k^{3}$-versus-$k^{1}$ tension between analytic predictions and
simulations therefore cannot be resolved on the spatial axis.
\del{We localise it instead to a single assumption, $\Omega_{T}\propto\Omega_{M}$,
which the self-convolution of the stress forbids.}
\why{removed: pre-empted by RoperPol:2019wvy/Sharma:2022ysf and retracted by our own
Sec.~V; replaced with the band statement below}
\add{It is not a
disagreement about physics but about band: $k^{3}$ is the asymptotic law and
$k^{1}$ the finite-duration regime above the break $k_{\rm br}\simeq1/\delta t$
\cite{Caprini:2009fx,RoperPol:2022iel,RoperPol:2023dzg} \add{(for a source shorter than
a Hubble time; for a longer one the linear regime begins at $\mathcal{H}_\ast$ and the
transition is logarithmic)}, and every accessible band lies on the $k^{1}$ side---
\add{in observed frequency the break falls below LISA's energy-density window for an
electroweak source across essentially the whole parameter space, so the causal
asymptote sits below the instrument as well as below every simulation box}.}
Second, the displacement of the GW peak to the source scale requires only that the
source correlator have a kink at zero time lag, a condition met by any source with
a finite lifetime and not specific to any decorrelation model; all cusped
correlators collapse onto a single prediction, so the only question is
differentiability at zero lag. This is conditional on a measurement not yet made:
the hydrodynamic UETC is Gaussian \cite{Auclair:2022jod}, and the magnetic
\emph{anisotropic-stress} UETC of a decaying, magnetically dominated cosmological source
has not been measured. Eulerian space--time correlations of forced MHD turbulence
\emph{have} been measured \cite{Perez:2020}, and they favour sweeping---which would
give $f'(0^{+})=0$ and hence tell against the cusp. We close by arguing
that the remaining freedom in the pipeline lies almost entirely in the third
input: the dissipation microphysics of the early Universe is neither
Navier--Stokes nor unit magnetic Prandtl number, and no existing calculation or
simulation of the GW signal uses the correct one.
\add{The field's own review of the relevant magnetohydrodynamics
\cite{RoperPol:2025lgc} sets free-streaming aside explicitly and does not treat the
magnetic Prandtl number at all.}
\del{---an omission acknowledged in the field's own review of the relevant
magnetohydrodynamics.}
\why{the abstract still said ``acknowledged''; citation verification established that
the review is declaring its scope rather than conceding a gap, and has no Pm content ---
Sec.~\ref{sec:outlook} was corrected in the first pass and the abstract was not}
We identify that gap and set out how to close it, but do not close it here.
\end{abstract}
...
every corner but one. Recomputing the LISA energy-density sensitivity from the
SciRD analytic noise model gives a minimum of $\Omega_{\rm sens}$ at
...
$8\,\mu$Hz--$0.8\,$mHz and the eddy-lifetime break $5\,\mu$Hz--$15\,$mHz. LISA therefore
observes on the $k^{1}$ side of the break, and the causal $k^{3}$ asymptote lies below the
instrument---the same conclusion Sec.~\ref{sec:cps-reproduction} reaches for the
simulation box, reached independently for the detector. The single corner that pushes the
break into the LISA window, $\gamma\sim10^{-3}$ with $u_0\sim0.3$, puts
$p_{\rm break}=0.94$ at the spectral knee itself, where the break is not separable from
the peak. Third, for a QCD source the break spans $5.5\,$nHz--$17\,\mu$Hz and so overlaps
...
\texttt{Notebooks/k\_break\_frequency.py}); the primary reference for that fit still
needs a bibliography entry, which could not be added in this pass because the literature
databases were unreachable from the machine the revision was prepared on. AUTHORS: please
add it}
...
The spatial axis is closed: no choice of $S$ moves the infrared
...
\end{equation}
a constant \emph{independent of $p$}. We reproduce it to better than $0.5\%$ at every
$p$ tested and for $M=0.05$--$0.5$, $R=10^{3}$--$10^{4}$.
...
\begin{enumerate}\itemsep2pt
  ...
  below every observationally accessible band \emph{and} below the fundamental mode
  of every published simulation box, so no simulation of this class can exhibit the
  ...
  has not been made, and this conclusion is conditional on it.
...
why the simulated $k^{1}$ of Sec.~\ref{sec:principal-discrepancy} survives every
treatment in Table~\ref{tab:three-works}. What the temporal history \emph{does} change
