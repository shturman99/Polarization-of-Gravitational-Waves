...
integrals in the master formula \eqref{eq:master} collapse \emph{without
approximation}, yielding a GW spectrum that is purely spatial in character and
entirely free of Mach-number or decorrelation-time parameters.
...
independent of time. Taking the two-point average of Eq.~\eqref{eq:uetc} with
the ansatz \eqref{eq:impulsive-stress}, the UETC becomes
\begin{equation}
  \Pi(k;\tau_1,\tau_2) = P(k)\,\delta(\tau_1-\tau_s)\,\delta(\tau_2-\tau_s),
  \label{eq:uetc-impulsive}
\end{equation}
where $P(k)$ is the spatial power spectrum of the burst amplitude. For the
monochromatic spatial model, $P(k)$ is determined by the same kinematic
decomposition that underlies Eq.~\eqref{eq:angular-delta}: the two $\delta$-functions
in $k$ pin both loop momenta to $k_0$, giving $P(k)\propto
E_0^2\,\mathcal{K}_0(p)/(k k_0^2)$ with $p=k/k_0$ restricted to $p\leq2$ by
the triangle inequality. No decorrelation model ($\eta_k$, $\hat g$, or any
finite coherence time) is needed; the UETC of Eq.~\eqref{eq:uetc-impulsive} is
exact and requires no further closure assumption.

\subsection{Temporal integral}
\label{sec:impulsive-temporal}

Inserting the UETC \eqref{eq:uetc-impulsive} into the master formula \eqref{eq:master},
\begin{align}
  \frac{d\rho_{\rm GW}}{d\ln k}
  &\;\propto\; k^3\!\int\!d\tau_1\!\int\!d\tau_2\,
    \cos[k(\tau_1-\tau_2)]\,
    P(k)\,\delta(\tau_1-\tau_s)\,\delta(\tau_2-\tau_s)
    \nonumber\\
  &= k^3\,P(k)\,\cos[k(\tau_s-\tau_s)]
  \;=\; k^3\,P(k)\,\cos(0)
  \;=\; k^3\,P(k).
  \label{eq:impulsive-temporal}
\end{align}
The temporal factor is identically unity regardless of $k$, $k_0$, or $\tau_s$.
This is the sharpest possible contrast with the stationary and decaying cases of
Sec.~\ref{sec:kraichnan-k41} and Sec.~\ref{sec:delta-decay}: whereas those
models produce a non-trivial temporal power spectrum $\widetilde\Pi(k,\omega)$
that peaks at a decorrelation frequency and suppresses the integrand for
$\omega\gtrsim\eta_0$, the impulsive UETC contributes the same weight to every
on-shell frequency. The GW spectrum is therefore entirely shaped by the
\emph{spatial} correlator $P(k)$, with no temporal modulation whatsoever.

\subsection{Spatial reduction}
\label{sec:impulsive-spatial}

With the temporal factor reduced to unity, Eqs.~\eqref{eq:angular-delta}--\eqref{eq:K0}
supply the spatial reduction without modification:
\begin{equation}
  \frac{d\rho_{\rm GW}}{d\ln k} \;\propto\; k^3\,P(k)
  \;\propto\; \frac{k^3\,E_0^2\,\mathcal{K}_0(p)}{k\,k_0^2}\,\Theta(2-p)
  \;=\; E_0^2\,k_0^2\,p^2\,\mathcal{K}_0(p)\,\Theta(2-p),
\end{equation}
where $\mathcal{K}_0(p) = \tfrac{14}{3}-\tfrac{p^2}{3}+\tfrac{p^4}{12}$ is the
geometric factor from Eq.~\eqref{eq:K0} evaluated at $k_1=u=k_0$. The
dimensionless kernel is
\begin{equation}
  \boxed{
...
\end{equation}
There is no remaining integral, no $q$ argument, and no $M$ dependence. In
$p$-shape this kernel coincides with the Kraichnan monochromatic kernel
\eqref{eq:dimless-delta-Kraichnan} evaluated at $q=0$: setting $q\to0$ in the
Gaussian $e^{-q^2/M^2}$ reproduces Eq.~\eqref{eq:dimless-impulsive}, which is
physically correct since the impulsive source has no characteristic decorrelation
frequency to distinguish any particular on-shell $\omega$.
...
\begin{equation}
  \Omega_{\rm GW}(p) \;\propto\; p^3\,\mathfrak{H}^{(\delta\tau,\delta k)}(p)
  = p^2\,\mathcal{K}_0(p)\,\Theta(2-p).
  \label{eq:omega-impulsive}
\end{equation}

\paragraph{Infrared slope.}
Expanding $\mathcal{K}_0(p)=\tfrac{14}{3}+O(p^2)$ as $p\to0$,
\begin{equation}
  \Omega_{\rm GW}(p) \;\xrightarrow{p\to0}\; \tfrac{14}{3}\,p^2,
\end{equation}
so the infrared slope is $\Omega_{\rm GW}\propto p^2$, not the causal $p^3$
that arises for spectrally extended sources. The $1/p$ in
$\mathfrak{H}^{(\delta\tau,\delta k)}$ comes from the angular phase-space factor
in Eq.~\eqref{eq:angular-delta}, which persists independently of the temporal
model; the same IR slope appears in the Kraichnan monochromatic kernel
\eqref{eq:dimless-delta-Kraichnan}.
...
\end{equation}
Setting the bracket to zero requires $3p^4-8p^2+56=0$, which has discriminant
$\Delta=64-4\cdot3\cdot56=-608<0$: no real roots. Since the bracket evaluates to
$56/3>0$ at $p=0$ and to $72>0$ at $p=2$, and has no roots, $f'(p)>0$
throughout $(0,2)$. The spectrum is therefore strictly increasing and peaks at
the triangle-inequality hard cutoff,
\begin{equation}
  p_{\rm peak} = 2 \qquad\Longrightarrow\qquad k_{\rm GW}^{\rm peak} = 2k_0.
  \label{eq:impulsive-peak}
\end{equation}
Geometrically, the maximum GW wavenumber reachable by combining two fluid modes
at $k_0$ is exactly $2k_0$; the monotone spatial kernel drives all power toward
this upper limit. The Kraichnan and decay kernels approach $p=2$ only in the
limit $M\to\infty$ or at large Reynolds number; the impulsive case reaches it
...
GW}(p)\equiv0$ for $p>2$. There is no UV power-law tail whatsoever, in contrast
to the smooth Gaussian (Kraichnan) or power-law (decay) falloffs of the
stationary models.

\subsection{Post-burst free propagation}
\label{sec:impulsive-propagation}

For $\tau>\tau_s$ the source is off. Inserting Eq.~\eqref{eq:impulsive-stress}
into the Green-function solution \eqref{eq:greens},
\begin{equation}
  h_{ij}(\kb,\tau) = \frac{16\pi G}{k}\,\sin[k(\tau-\tau_s)]\,A^{\rm TT}_{ij}(\kb),
  \qquad \tau>\tau_s,
  \label{eq:impulsive-h}
\end{equation}
and $\dot h_{ij}(\kb,\tau)=16\pi G\cos[k(\tau-\tau_s)]\,A^{\rm TT}_{ij}(\kb)$.
Each mode oscillates on-shell at $\omega=k$ with amplitude fixed by the burst;
time-averaging $\langle\cos^2\rangle=\tfrac{1}{2}$ in the Isaacson formula
returns the frozen spectrum \eqref{eq:omega-impulsive}. There is no further
evolution: the GW background is impressed at $\tau_s$ and decouples from the
source permanently. This is the temporal analog of the spatial hard cutoff --
the impulsive source produces the maximum-wavenumber ($2k_0$) GW spectrum
in a single instant.
